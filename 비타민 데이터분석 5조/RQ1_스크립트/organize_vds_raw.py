...
입력: 교통데이터_원본/vds_downloads/VDS_{노선}_{구간}_{시작일}_{종료일}.csv
      (CP949 인코딩, 컬럼: 구간,일자,교통량  <- 이미 "일자별 총교통량"으로 걑계된 값,
       시간대별 데이터 아님)
...
AXIS_TCS_STATIONS 매핑 (2026-08-16 팀 논의로 확정):
  - 남해2지선/남해선/중앙선(=제1배후도로 접속점): 가락·대동 TCS 그대로 사용
  - 부산신항선(=제2배후도로): 원래 남진례·대청·진해·진해본선·진례 5개 영업소를 "같은 축"
    후보로 잡아뒀었지만, 실제로 데이터가 있는 곳은 진례뿐이라 팀에서 "진례 TCS 화물차비중을
    채택"하기로 명시적으로 확정함. 그래서 부산신항선은 진례 단독으로 고정.
    (참고: 부산신항선은 화물차비중 표본이 진례 하나뿐이라 부족하다고 판단해서, 별도로
     correlation_analysis.py 버전2에서는 부산신항선만 화물차환산_통행량 대신 이 스크립트가
     출력 1로 저장하는 원시 총교통량(VDS_구간별_시간대별, 전 차종 포함)을 사용함.)
"""
...
# 어떤 노선이 어떤 TCS 영업소(들)와 "같은 축"인지
# (부산신항선은 2026-08-16 팀 논의로 "진례 채택" 확정 - 더 이상 5개 영업소 평균이 아님)
AXIS_TCS_STATIONS = {
    "남해2지선": ["가락"],
    "남해선(순천-부산)": ["대동"],
    "중앙선": ["대동"],
