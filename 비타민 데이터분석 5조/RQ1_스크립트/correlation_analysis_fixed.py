...
SEOKEON_VALID_FROM = "2024-03-09"  # 서컨(7부두) 개장일 — 이전 구간은 미개장(0 관측)이므로 결측 처리
...
def load_cargo_volume(path: Path) -> pd.DataFrame:
    ...
    # [2026-08-21 수정] 원본 CSV(물동량_상관분석용.csv)의 서컨물동량 컬럼은 개장 전
    # 구간이 NaN이 아니라 0.0으로 채워져 있다 (직접 확인: 2024-03-09 이전 798행 중
    # 795행이 정확히 0.0). 이 스크립트는 pairwise dropna만 하므로 0.0은 "실제 관측값"
    # 으로 그대로 상관계수 계산에 들어가서, 개장 전/후 물동량 수준 차이가 마치 도로
    # 변수와 강한 상관이 있는 것처럼 보이게 만드는 인위적인 효과를 만든다(회귀분석
    # 단계에서 발견된 것과 동일한 문제). 원본 CSV를 고칠 수 없는 상황을 가정해,
    # 이 스크립트에서 매번 개장일 기준으로 다시 결측 처리한다.
    if "서컨물동량" in df.columns:
        before_mask = df.index < SEOKEON_VALID_FROM
        n_zeroed = int(((df.loc[before_mask, "서컨물동량"] == 0) | df.loc[before_mask, "서컨물동량"].notna()).sum())
        df.loc[before_mask, "서컨물동량"] = pd.NA
        df["서컨물동량"] = pd.to_numeric(df["서컨물동량"], errors="coerce")
        print(f"[결측 처리] 서컨물동량: {SEOKEON_VALID_FROM} 이전 {n_zeroed}행을 결측(NaN) 처리함 "
              f"(원본 CSV에는 0.0으로 채워져 있었음 — 상관계수 계산에 그대로 섞여 들어가는 것을 방지)")
